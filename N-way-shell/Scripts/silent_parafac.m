function [Factors,it,err,corcondia,FactorsPerIteration,ErrorPerIteration]=silent_parafac(X,Fac,Options,const,OldLoad,FixMode,Weights);
...
while (((f>crit) | (norm(connew-conold)/norm(conold)>MissConvCrit) | ConstraintsNotRight) & it<maxit)|~ nonneg_obeyed
    ...
    for mode=1:numModes
        FactorsPerIteration{mode}(:,:,it) = ff{mode};
        ErrorPerIteration(it) = err;
    end
