...
if strcmp(lower(extractBetween(Method, 1, 3)),'tuc')
   if length(FacMin)==1
...
if Show&FacMin-FacMax~=0
   ...
   figure      
   save jjj
   if lower(extractBetween(Method, 1, 3))~='tuc'
...
if lower(extractBetween(Method, 1, 3)) == 'par'
...
elseif lower(extractBetween(Method, 1, 3)) == 'tuc'
   Xc = reshape(X- ones(I,1)*Mean',DimX);
   if exist('parameters')==1
...
while ~Conv
   ...
   if extractBetween(Method, 1, 3) == 'par'
      fact = parafac(reshape(Xcent,DimX),f,[1e-2 0 0 0 NaN maxit],[],fact);
      M = reshape(nmodel(fact),DimX(1),prod(DimX(2:end)));
   elseif extractBetween(Method, 1, 3) == 'tuc'
