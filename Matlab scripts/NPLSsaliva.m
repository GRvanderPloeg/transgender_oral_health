...
% Prepare y
% SHBG = column 22
% TESTOSTERONE = column 23
% ESTROGEN = column 20

mask0 = (sampleInfo(:,13) == "0");
mask3 = (sampleInfo(:,13) == "3");
mask6 = (sampleInfo(:,13) == "6");
mask12 = (sampleInfo(:,13) == "12");

testosterone0 = sampleInfo(mask0, [12 23]);
testosterone0 = testosterone0(testosterone0(:,2) ~= "NA",:); % remove NAs
testosterone3 = sampleInfo(mask3, [12 23]);
testosterone3 = testosterone3(testosterone3(:,2) ~= "NA",:); % remove NAs
testosterone6 = sampleInfo(mask6, [12 23]);
testosterone6 = testosterone6(testosterone6(:,2) ~= "NA",:); % remove NAs
testosterone12 = sampleInfo(mask12, [12 23]);
testosterone12 = testosterone12(testosterone12(:,2) ~= "NA",:); % remove NAs

subsetSubjects0 = ismember(sampleInfo(:,12), testosterone0(:,1));
subsetSubjects3 = ismember(sampleInfo(:,12), testosterone3(:,1));
subsetSubjects6 = ismember(sampleInfo(:,12), testosterone6(:,1));
subsetSubjects12 = ismember(sampleInfo(:,12), testosterone12(:,1));

% For 0m, 3m
usableSubjects = unique(sampleInfo(subsetSubjects0 & subsetSubjects3, 12));
...
path = "./20240506_NPLS_rerun/Saliva_original/";
...
[saliva_cube, salivaSubjectMeta, salivaConditionMeta] = rawDataToCube(saliva_filtered, sampleInfo(:,12), str2double(sampleInfo(:,13)), keepIndividuals);

%%
% Remove subjects with missing testosterone measurements
mask = ismember(salivaSubjectMeta, testosterone(:,1));
...
saliva_subjectMeta_filtered = sortrows(unique(sampleInfo(:,[12 7]), "rows"), 1);
